...
function evaluateFieldDependents(field: FormField, values: any, context: FormContextProps) {
  ...
  if (field.fieldDependents) {
    field.fieldDependents.forEach((dep) => {
      ...
      if (dependent.questionOptions.calculate?.calculateExpression) {
        evaluateAsyncExpression(
          ...
          .then((result) => {
            ...
            if (!dependent.meta.submission) dependent.meta.submission = {};
      ...
      // --- evaluate hide ---
      if (dependent.hide) {
        const wasHidden = dependent.isHidden;
        ...
        // --- Deselect value if it just became hidden ---
        if (!wasHidden && dependent.isHidden) {
          setValue(dependent.id, null);
        }

        // --- Re-enable field if it becomes visible again ---
        if (wasHidden && !dependent.isHidden) {
          dependent.isDisabled = false;
          dependent.readonly = false;

          // Re-evaluate disable expressions if present
          ...
          // Re-evaluate readonly expressions
          if (dependent.meta.readonlyExpression) {
        ...
        if (targetSection) {
          targetSection.questions = targetSection.questions.map((question) =>
            question.id === dependent.id ? dependent : question,
          );
      ...
      if (dependent.validators?.some((validator) => validator.type === 'conditionalAnswered')) {
        const fieldValidatorConfig = dependent.validators.find((v) => v.type === 'conditionalAnswered');
      ...
      // --- evaluate hide/disabled for answers ---
      ...
      dependent.questionOptions.answers
        ?.filter((answer) => !isEmpty(answer.disable?.disableWhenExpression))
        .forEach((answer) => {
          answer.disable.isDisabled = evaluateExpression(
            answer.disable?.disableWhenExpression,
            { value: dependent, type: 'field' },
  ...
  if (field.sectionDependents) {
    field.sectionDependents.forEach((sectionId) => {
      for (let i = 0; i < formJson.pages.length; i++) {
        const section = formJson.pages[i].sections.find((s) => s.label === sectionId);
        if (section) {
          evaluateHide(
