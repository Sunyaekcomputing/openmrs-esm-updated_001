...
export class CommonExpressionHelpers {
  ...
  setFieldValue = (questionId: string, value: any) => {
    console.log(`Setting value for field "${questionId}" to:`, value);
    this.allFieldValues[questionId] = value;
  
    // Optional: trigger any dependents or re-render logic if needed
    const targetField = this.allFields.find((f) => f.id === questionId);
    if (targetField && targetField.fieldDependents) {
      console.log(`Field "${questionId}" has dependents:`, Array.from(targetField.fieldDependents));
      // Here you could notify dependent fields if your form engine supports it
    }
  };
  updateRadioOnCheckboxChange = (checkboxId: string, radioId: string) => {
    console.log(`updateRadioOnCheckboxChange called with checkboxId: "${checkboxId}", radioId: "${radioId}"`);
  
    const selectedCheckboxes = this.useFieldValue(checkboxId) || [];
    console.log(`Selected checkboxes for "${checkboxId}":`, selectedCheckboxes);
  
    const targetValue = '884AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';
  
    if (Array.isArray(selectedCheckboxes) && selectedCheckboxes.includes(targetValue)) {
      console.log(`Target value "${targetValue}" is selected. Resetting radio field "${radioId}" to 0.`);
      this.setFieldValue(radioId, 0);
    } else {
      console.log(`Target value "${targetValue}" is NOT selected. Radio field "${radioId}" remains unchanged.`);
    }
  };
