...
// Removed launchStartVisitPrompt import since we won’t call it
...
export function launchFormEntryOrHtmlForms(
  ...
  // Allow launching form even if no visitUuid provided
  ...
  if (htmlForm) {
    // Launch HTML form without visitUuid if missing
    launchHtmlFormEntry(
      patientUuid,
      formName,
      encounterUuid ?? '',
      visitUuid ?? '', // pass empty string if missing
      htmlForm,
      htmlFormEntryWorkspaceName,
    );
  } else {
    // Launch JSON form without visit details if missing
    launchFormEntry(
      formUuid,
      patientUuid,
      encounterUuid,
      formName,
      visitUuid, // can be undefined or null
      visitTypeUuid,
      visitStartDatetime,
      visitStopDatetime,
      htmlForm,
 ...
 * @returns
 */
function toHtmlForm(form: Form, htmlFormEntryForms: Array<HtmlFormEntryForm>): HtmlFormEntryForm | null {
  const isHtmlForm =
